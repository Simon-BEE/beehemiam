...
@section('content')
    ...
    <p style="font-size: 16px; font-weight: bold; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">Nous vous informons que votre commande du <strong>{{ $order->created_at->format('d/m/Y à H:i') }}</strong> passée sur <strong>Beehemiam.fr</strong> est annulée.</p>

    <p style="font-size: 16px; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">Cette commande <strong>n°{{ $order->id }}</strong> d'un montant de <strong>{{ $order->formatted_price }}€</strong> est annulée. Vous recevez le remboursement dans les plus brefs délais.</p>
    ...
    <table class="body-action" align="center" width="100%" cellpadding="0" cellspacing="0" role="presentation" style="width: 100%; -premailer-width: 100%; -premailer-cellpadding: 0; -premailer-cellspacing: 0; text-align: center; margin: 30px auto; padding: 0;">
      ...
      <p style="font-size: 12px; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">Ce message fait suite à votre demande du {{ $order->updated_at->format('d/m/Y à H:i') }}.</p>
