...
@section('content')
    ...
    <p style="font-size: 16px; font-weight: bold; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">Nous vous informons que votre commande du <strong>{{ $refund->order->created_at->format('d/m/Y à H:i') }}</strong> passée sur <strong>Beehemiam.fr</strong> est remboursée d'un montant de {{ $refund->formatted_amount }}€.</p>

    <p style="font-size: 16px; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">Le montant complet de cette commande <strong>n°{{ $refund->order->id }}</strong> est <strong>{{ $refund->order->formatted_price }}€</strong>. Vous recevez le remboursement de <strong>{{ $refund->formatted_amount }}€</strong> dans les plus brefs délais.</p>

    @if ($refund->order->formatted_total_with_refund == 0)
    ...
    <p style="font-size: 16px; line-height: 1.625; color: #51545E; margin: .4em 0 1.1875em;">Votre argent sera disponible sur votre compte dans un délai de 7 à 14 jours, selon les banques, à partir de la date de réception de ce mail.</p>
