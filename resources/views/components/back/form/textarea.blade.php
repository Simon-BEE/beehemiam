...
<div class="{{ $classDiv ?? 'w-full mb-4' }}">
    ...
    <textarea
        name="{{ $name }}"
        id="{{ $id ?? $name }}"
        class="w-full h-60 mt-2 px-4 py-2 block rounded border-gray-200 dark:border-transparent dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray @error($name) border-red-500 @enderror {{ $class ?? '' }}"
