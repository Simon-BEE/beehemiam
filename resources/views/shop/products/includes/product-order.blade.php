<div class=" flex flex-col space-y-6">
    ...
    @else
        <article class="bg-primary-200 p-8 rounded w-full md:w-2/3">
            <p class="font-bold">Le vêtement est indisponible.</p>
            @auth
                <x-form.form method="POST" action="{{ route('shop.products.notify-availability', $currentOption) }}">
                    <button type="submit" class="mt-4 p-5 rounded-lg bg-primary-100 inline-flex items-center text-primary-500 transition-colors duration-200 hover:bg-primary-500 hover:text-white">
                        <svg class="w-4 h-4 mr-2" viewBox="0 0 24 24">
            ...
            @else
                ...
                >
                    <svg viewBox="0 0 24 24" class="w-4 h-4 mr-2">
                        <path
                        fill="currentColor"
