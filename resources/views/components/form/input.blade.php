...
<div class="{{ $classDiv ?? 'w-full mb-4' }}">
    ...
    <input
        ...
        name="{{ $name }}"
        id="{{ $name }}"
        class="w-full mt-2 px-4 py-2 block rounded bg-primary-200 text-kaki-900 border border-transparent focus:bg-white focus:outline-none focus:border-transparent focus:ring-2 focus:ring-primary-500 @error($name) border-red-500 @enderror {{ $class ?? '' }}"
